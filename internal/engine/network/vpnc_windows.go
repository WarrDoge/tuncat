...
func ConfigInterface(ctx *engine.Context, cSess *enginesession.ConnSession, dev device.Device) error {
	state := routingState(ctx)
	native, ok := dev.(*device.NativeTun)
	if !ok {
	...
	prefixVPN, _ := netip.ParsePrefix(netutil.IpMask2CIDR(cSess.VPNAddress, cSess.VPNMask))
...
func SetRoutes(ctx *engine.Context, cSess *enginesession.ConnSession) error {
	...
	if !state.staleRollbackClean {
		if err := cleanupStaleRollback(state); err != nil {
			engineconfig.Warn("stale Windows rollback cleanup failed:", err)
		}
...
func GetLocalInterface(ctx *engine.Context) error {
	...
	for _, ifc := range ifcs {
		engineconfig.Debug(ifc.AdapterName(), ifc.Description(), ifc.FriendlyName(), ifc.Ipv4Metric, ifc.IfType)
	...
	engineconfig.Info("GetLocalInterface:", primaryInterface.AdapterName(), primaryInterface.Description(),
...
func setDNS(ctx *engine.Context, cSess *enginesession.ConnSession) error {
	...
	if !state.dnsStateCaptured {
		if current, err := state.iface.DNS(); err == nil {
			...
			if err := persistRollbackState(state); err != nil {
				engineconfig.Warn("persist Windows rollback state failed:", err)
			}
		}
	}
	...
	if err := applyNRPTRules(domains, servers); err != nil {
		engineconfig.Warn("NRPT configuration failed:", err)
	}

	return nil
}

func restoreDNS(ctx *engine.Context) {
	state := routingState(ctx)
	if err := clearOwnedNRPTRules(); err != nil {
		engineconfig.Warn("NRPT cleanup failed:", err)
	}

	if state.dnsStateCaptured {
		if err := state.iface.SetDNS(windows.AF_INET, state.previousDNSServers, []string{}); err != nil {
			engineconfig.Warn("restore DNS servers failed:", err)
		}
		state.previousDNSServers = nil
		state.dnsStateCaptured = false
	}
	if err := clearRollbackState(); err != nil {
		engineconfig.Warn("clear Windows rollback state failed:", err)
	}
}
