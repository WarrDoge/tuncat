...
func ConfigInterface(ctx *engine.Context, cSess *enginesession.ConnSession, _ device.Device) error {
	...
	addr, _ := netlink.ParseAddr(netutil.IpMask2CIDR(cSess.VPNAddress, cSess.VPNMask))
...
func setDNS(ctx *engine.Context, cSess *enginesession.ConnSession) {
	if len(cSess.DNS) == 0 {
		return
	}
	state := routingState(ctx)
	...
	if !state.resolvBackupOK {
		if err := os.MkdirAll("/run/tuncat", 0o700); err != nil {
			engineconfig.Warn("create /run/tuncat failed:", err)
		} else if err := netutil.CopyFile(linuxResolvBackup, linuxResolvConfPath); err == nil {
			state.resolvBackupOK = true
		} else {
			engineconfig.Warn("backup resolv.conf failed:", err)
		}
	}
	...
	if err != nil {
		engineconfig.Error("set DNS failed:", err)
	}
}
...
func restoreDNS(ctx *engine.Context, cSess *enginesession.ConnSession) {
	...
	if state.resolvBackupOK {
		if err := netutil.CopyFile(linuxResolvConfPath, linuxResolvBackup); err != nil {
			engineconfig.Warn("restore resolv.conf failed:", err)
		} else {
