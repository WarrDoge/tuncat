...
func init() {
	svcName := "tuncat-engine"
	if runtime.GOOS == "windows" {
		svcName = "TuncatEngine"
	}
	serviceConfig = &service.Config{
		Name:        svcName,
		DisplayName: "Tuncat Engine Service",
		Description: "Tuncat embedded engine service",
	}
}
...
func (p *program) run() {
	...
	ctx.Logger = engineconfig.InitLog(ctx.Cfg)
	p.server = control.NewRPCServer(ctx)
	if err := p.server.Setup(); err != nil {
		engineconfig.Fatal(err)
	}
}
