...
func initTunnel(ctx *vpncore.VPNContext) map[string]string {
	mtu := ctx.Cfg.BaseMTU
	if mtu <= 0 {
		mtu = 1200
	}
	...
	ctx.Session.PreMasterSecret, _ = utils.MakeMasterSecret()
...
func SetupTunnel(ctx *vpncore.VPNContext) error {
	if ctx == nil {
