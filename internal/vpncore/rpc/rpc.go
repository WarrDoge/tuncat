...
type RPCServer struct {
	...
	ctx             *vpncore.VPNContext
	httpServer      *http.Server
}

func NewRPCServer(ctx *vpncore.VPNContext) *RPCServer {
...
func (server *RPCServer) Context() *vpncore.VPNContext {
	return server.ctx
}
...
func (server *RPCServer) Setup() error {
	...
	go func() {
		...
		if err != nil && err != http.ErrServerClosed {
			base.Error("rpc listen failed:", err)
		}
	}()

	base.Info("rpc server listening on", addr)
	return nil
}
...
func (server *RPCServer) rpc(resp http.ResponseWriter, req *http.Request) {
	...
	rpcConn := jsonrpc2.NewConn(req.Context(), jsonStream, server, jsonrpc2.SetLogger(base.GetBaseLogger()))
...
func (server *RPCServer) Handle(ctx context.Context, conn *jsonrpc2.Conn, req *jsonrpc2.Request) {
	...
	case CONFIG:
		...
		server.ctx.Logger = base.InitLog(server.ctx.Cfg)
