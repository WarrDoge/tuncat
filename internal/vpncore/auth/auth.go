...
func InitAuth(ctx *vpncore.VPNContext) error {
...
func PasswordAuth(ctx *vpncore.VPNContext) error {
	if err := ensureContext(ctx); err != nil {
		return err
	}
	prof := ctx.Profile
...
func tplPost(ctx *vpncore.VPNContext, typ int, path string, dtd *proto.DTD) error {
	prof := ctx.Profile
	authState := ctx.Auth
	cfg := ctx.Cfg
	...
	base.Debug("auth POST", url)
	req, _ := http.NewRequest("POST", url, tplBuffer)

	utils.SetCommonHeader(req, cfg.AgentName, cfg.AgentVersion, cfg.CiscoCompat)
	if cfg.CiscoCompat {
...
func closeAuthConn(authState *vpncore.AuthState) {
...
func resolveRequestPath(prof *vpncore.Profile, path string) string {
...
func requestHostForURL(prof *vpncore.Profile) string {
	if prof == nil {
		return "localhost"
	}
...
func setAuthPlatform(authState *vpncore.AuthState, prof *vpncore.Profile, platform string) {
